...
function StatCard({ title, value, sub, accent, icon, positive }: StatCardProps) {
  return (
...
export default function SellerPage() {
  ...
  const fetchData = useCallback(async (uid: string) => {
    ...
    // Only fetch MY clients (assigned_to = my user id)
  ...
  // Revenue = sum of budget_usd for my clients marked as bought (my personal sales)
  const monthRevenue   = myClients
    .filter((c) => c.status === "bought")
    .reduce((s, c) => s + (c.budget_usd ?? 0), 0);
  const conversionRate =
    myClients.length > 0
      ? ((boughtCount / myClients.length) * 100).toFixed(0)
      : "0";

  // ── Filtering ─────────────────────────────────────────────────────────────

  const filtered = myClients.filter(
      ...
      >
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-sm font-semibold text-white">My Stats</h1>
          </div>
            ...
            <div className="rounded-xl border p-5"
              ...
              <div className="space-y-3">
                {ALL_STATUSES.map((s) => {
                  ...
                  const pct   = myClients.length > 0
                    ? Math.round((count / myClients.length) * 100)
                    : 0;
                  return (
                              ...
                              <div className="flex flex-col gap-0.5">
                                {client.phone && (
                                  <a href={`tel:${client.phone}`} className="text-xs"
                                    style={{ color: "#64748b" }}>{client.phone}</a>
                                )}
                                {client.email && (
                                  <a href={`mailto:${client.email}`} className="text-xs"
                                    style={{ color: "#64748b" }}>{client.email}</a>
                                )}
